...
public class CreateViewController {
	...
	public void addEvent(ActionEvent event) {
		if (inName.equals("")) {
			...
			inCounter++;
			ingredientList.add(cookbook.ingredients.get(inId).toString());
			ingredientsList.setItems(ingredientList);
		}
	}
	...
	public void minusEvent(ActionEvent event) {
		...
		inId--;
		inCounter--;
		ingredientList.remove(ingredientsList.getSelectionModel().getSelectedItem());
		ingredientsList.setItems(ingredientList);
		// TODO Autogenerated
	}
	...
	public void choosePicButton(ActionEvent event) {
		...
		if (f != null) {
			pictureLabel.setText(f.getAbsolutePath());
	...
	public void confirmEvent(ActionEvent event) {
		if (reName.equals("")) {
			...
			if (result.get() == ButtonType.YES) {
				cookbook.recipeList.add(new Recipe(reName, servePeople, cookTime, steps));
				for (int i = inId; i > inId - inCounter; i--) {
					cookbook.recipeList.get(cookbook.recipeList.size() - 1).addIngredient(cookbook.ingredients.get(i));
				}
				cookbook.nowRecipe = cookbook.recipeList.get(cookbook.recipeList.size() - 1);

				try {
					cookbook.nowRecipe.setID(RecipeDAO.getRecipeByName(cookbook.nowRecipe.getName()).getID());
					for (Ingredient i : cookbook.nowRecipe.getIngredientList()) {
						cookbook.nowRecipe.addIngredient(i);
					}
					RecipeDAO.addNewRecipe(cookbook.nowRecipe);
	...
	public void initialize() throws ClassNotFoundException, SQLException {
		...
		categoryBox.getItems().addAll(CategoryDAO.getAllCategoryString());
